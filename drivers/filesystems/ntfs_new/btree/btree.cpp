...
CompareTreeKeys(PBTreeKey Key1, PBTreeKey Key2, BOOLEAN CaseSensitive)
{
    ...
    Key1Name.Length = Key1Name.MaximumLength
        = GetFileName(Key1)->NameLength * sizeof(WCHAR);

    Key2Name.Buffer = GetFileName(Key2)->Name;
    Key2Name.Length = Key2Name.MaximumLength
        = GetFileName(Key2)->NameLength * sizeof(WCHAR);
                    ...
                    PBTree *NewTree)
{
    ...
    {
        DPRINT1("No index root attribute for MFT ID %ld!\n", File->Header->MFTRecordNumber);
        return STATUS_NOT_FOUND;
    }

    IndexRootExData = (PIndexRootEx)GetResidentDataPointer(IndexRootAttribute);
    CurrentOffset = IndexRootExData->Header.IndexOffset;
    Tree = new(NonPagedPool) BTree();
    RootNode = new(NonPagedPool) BTreeFilenameNode();
    CurrentKey = new(NonPagedPool) BTreeKey();
    ...
    while (CurrentOffset < IndexRootExData->Header.TotalIndexSize)
    {
        ...
        if (!CurrentKey->IndexEntry)
        {
            DPRINT1("ERROR: Couldn't allocate memory for current entry!\n");
            DestroyBTree(Tree);
            return STATUS_INSUFFICIENT_RESOURCES;
        }
        ...
        {
            // Create the next key
            NextKey = new(NonPagedPool) BTreeKey();
            if (!NextKey)
            {
                DPRINT1("ERROR: Couldn't allocate memory for next key!\n");
                DestroyBTree(Tree);
        ...
        // Copy the current entry to its key
        RtlCopyMemory(CurrentKey->IndexEntry,
                      CurrentNodeEntry,
                      CurrentNodeEntry->EntryLength);
